...
<div class="content lg:max-w-screen-lg lg:mx-auto py-8">
    ...
    <div class="my-6">
        ...
        <div class="flex justify-between items-center">
            <a href="index.php?page=builder/crud/create&data=<?=$_data?>" class="p-2 bg-green-500 text-white rounded">+ Add New</a>
            <div>
                <form action="" method="get">
                    ...
                    <div class="form-group inline-block <?=isset($_GET['kelurahan']) && $_GET['kelurahan'] ? '' : 'hidden' ?>" id="kelurahan">
                        <select name="kelurahan" class="p-2 w-full border rounded">
                            <option value="" selected readonly>- Pilih Kelurahan -</option>
                            <?php foreach($kelurahans as $kelurahan):?>
                                <option <?= isset($_GET['kelurahan']) && $_GET['kelurahan'] == $kelurahan['KD_KELURAHAN'] ? 'selected' : ''?> value="<?=$kelurahan['KD_KELURAHAN']?>"><?=$kelurahan['KD_KELURAHAN']." - ".$kelurahan['NM_KELURAHAN']?></option>
                            <?php endforeach ?>
                        </select>
                    </div>
                    ...
                    <tr class="border-b border-gray-200 hover:bg-gray-100">
                        ...
                        <td class="py-3 px-6 text-center">
                            <div class="flex item-center justify-center">
                                ...
                                <a href="index.php?page=builder/crud/edit&data=<?=$_data?>&key=<?=$key?>" class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                                    </svg>
                                </a>
                                <a href="index.php?action=builder/crud/delete&data=<?=$_data?>&key=<?=$key?>" class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110" onclick="if(confirm('Apakah anda yakin menghapus data ini')){return true}else{return false}">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<script>
    ...
    function kecamatanChange(el){
        fetch("index.php?page=builder/blok/index&filter-kecamatan="+el.value).then(response => response.json()).then(data => {
                ...
                var html = `<select name="kelurahan" class="p-2 w-full border rounded">
                            <option value="" selected readonly>- Pilih Kelurahan -</option>`

                data.map(dt=>{
                    html += `<option value="${dt.KD_KELURAHAN}">${dt.NM_KELURAHAN}</option>`
                })
