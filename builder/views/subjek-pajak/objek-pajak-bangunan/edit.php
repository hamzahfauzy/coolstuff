...
<div class="content lg:max-w-screen-lg lg:mx-auto py-8">
    <h2 class="text-4xl">Edit Objek Pajak Bumi : <?=$subjekPajak['NM_WP']?></h2>
...
&kecamatan=<?=$opBumi['KD_KECAMATAN']?>&kelurahan=<?=$opBumi['KD_KELURAHAN']?>&blok=<?=$opBumi['KD_BLOK']?>&znt=<?=$opBumi['KD_ZNT']?>" method="post" enctype="multipart/form-data">
                ...
                <div class="form-group mb-2">
                    ...
                    <select name="KD_KECAMATAN" class="p-2 w-full border rounded" onchange="kecamatanChange(this)">
                        ...
                        <?php foreach($kecamatans as $kecamatan):?>
                            <option <?= $opBangunan['KD_KECAMATAN'] == $kecamatan['KD_KECAMATAN'] ? 'selected' : ''?> value="<?=$kecamatan['KD_KECAMATAN']?>"><?=$kecamatan['KD_KECAMATAN']." - ".$kecamatan['NM_KECAMATAN']?></option>
                        <?php endforeach ?>
                    </select>
                </div>
                <div class="form-group mb-2 <?= $opBangunan['KD_KELURAHAN'] ? '' : 'hidden' ?>" id="kelurahan">
                    ...
                    <select name="KD_KELURAHAN" class="p-2 w-full border rounded" onchange="kelurahanChange(this)">
                        ...
                        <?php foreach($kelurahans as $kelurahan):?>
                            <option <?= $opBangunan['KD_KELURAHAN'] == $kelurahan['KD_KELURAHAN'] && $opBangunan['KD_KECAMATAN'] == $kelurahan['KD_KECAMATAN']  ? 'selected' : ''?> value="<?=$kelurahan['KD_KELURAHAN']?>"><?=$kelurahan['KD_KELURAHAN']." - ".$kelurahan['NM_KELURAHAN']?></option>
                        <?php endforeach ?>
                    </select>
                </div>
                <div class="form-group mb-2 <?= $opBangunan['KD_BLOK'] ? '' : 'hidden' ?>" id="blok">
                    <label>Blok</label>
                    <select name="KD_BLOK" class="p-2 w-full border rounded">
                        ...
                        <?php foreach($bloks as $blok):?>
                            <option <?= $opBangunan['KD_BLOK'] == $blok['KD_BLOK'] && $opBangunan['KD_KELURAHAN'] == $blok['KD_KELURAHAN'] && $opBangunan['KD_KECAMATAN'] == $blok['KD_KECAMATAN']  ? 'selected' : ''?> value="<?=$blok['KD_BLOK']?>"><?=$blok['KD_BLOK']?></option>
                        <?php endforeach ?>
                    </select>
                </div>
                <?php 
                ...
                <div class="form-group mb-2">
                    ...
                    <?= Form::input($val['data_type'], $val['column_name'], ['class'=>"p-2 w-full border rounded","value"=>$opBangunan[$val["column_name"]],"placeholder"=>$label,'maxlength'=>$val['character_maximum_length'] ]) ?>
                </div>
                <?php endforeach ?>
