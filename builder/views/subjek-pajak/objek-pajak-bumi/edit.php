...
<div class="content lg:max-w-screen-lg lg:mx-auto py-8">
    <div class="flex justify-between items-center mb-5">
        <h2 class="text-xl mr-3">Edit Objek Pajak Bumi : <?=$subjekPajak['NM_WP']?></h2>
     ...
     <form id="login-form" action="index.php?page=<?=$_GET['page']?>&id=<?=$_GET['id']?>&NOP=<?=$_GET['NOP']?>" method="post" enctype="multipart/form-data">
        ...
        <div class="bg-white shadow-md rounded my-6 p-8">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="form-group mb-2">
                    <label>NO SPOP</label>
                    <input type="text" class="p-2 mt-2 w-full border rounded" value="<?=isset($old) && $old["NO_SPOP"] ? $old["NO_SPOP"] : $opBumi["NO_FORMULIR"]?>" name="NO_SPOP">
                </div>
            ...
            <div class="grid grid-cols-3 gap-4">
                <div class="form-group mb-2">
                    <label>No Urut</label>
                    <input type="number" class="p-2 mt-2 w-full border rounded" value="<?=isset($old) && $old["NO_URUT"] ? $old["NO_URUT"] : $opBumi["NO_URUT"]?>" name="NO_URUT">
                </div>
                <div class="form-group mb-2">
                    <label>Kode</label>
                    <select name="KODE" class="p-2 mt-2 w-full border rounded">
                        <option value="" selected readonly>- Pilih Kode -</option>
                        <option <?= isset($old) && $old['KODE'] == '0' || $opBumi['KD_JNS_OP'] == '0' ? 'selected' : ''?>  value="0">0</option>
                        <option <?= isset($old) && $old['KODE'] == '7' || $opBumi['KD_JNS_OP'] == "7" ? 'selected' : ''?> value="7">7</option>
                    </select>
                </div>
        ...
        <div class="bg-white shadow-md rounded my-6 p-8">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="form-group mb-2">
                    <label>Jalan</label>
                    <select name="JALAN" class="p-2 w-full border rounded" id="jalan" onchange="fillZNT(this)">
                ...
                <div class="form-group mb-2" id="znt">
                    <label>ZNT</label>
                    <input type="text" class="p-2 w-full border rounded" value="<?=isset($old) && $old["KD_ZNT"] ? $old["KD_ZNT"] : $opBumi['KD_ZNT']?>" name="KD_ZNT" id="KD_ZNT" readonly>
                </div>
            </div>
            ...
            <div class="form-group mb-2">
                ...
                <select name="JNS_BUMI" class="p-2 w-full border rounded">
                    ...
                    <?php foreach($jenisBumis as $jenisBumi):?>
                        <option <?= (isset($old) && $old['JNS_BUMI'] == substr($jenisBumi,1,1)) || $opBumi['JNS_BUMI'] == substr($jenisBumi,1,1) ? 'selected' : ''?> value="<?=substr($jenisBumi,1,1)?>"><?=$jenisBumi?></option>
                    <?php endforeach ?>
                </select>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div class="form-group mb-2">
                    ...
                    <input type="number" class="p-2 mt-2 w-full border rounded" value="<?=isset($old) && $old["LUAS_TANAH"] ? $old["LUAS_TANAH"] : $opBumi['LUAS_BUMI']?>" name="LUAS_TANAH">
                </div>
            </div>
        ...
        <div class="bg-white shadow-md rounded my-6 p-8">
            <div class="grid grid-cols-2 gap-4 mt-4">
                ...
                <div class="form-group mb-2">
                    ...
                    <input type="date" class="p-2 mt-2 w-full border rounded" value="<?=isset($old) && $old["TGL_PENDATAAN"] ? $old["TGL_PENDATAAN"] : $datOP['TGL_PENDATAAN_OP']->format('Y-m-d')?>" name="TGL_PENDATAAN">
                </div>

                <div class="form-group mb-2">
                    <label>NIP Pendata</label>
                    <input type="text" class="p-2 mt-2 w-full border rounded" value="<?=isset($old) && $old["NIP_PENDATA"] ? $old["NIP_PENDATA"] : $datOP["NIP_PENDATA"]?>" name="NIP_PENDATA">
                </div>

                <div class="form-group mb-2">
                    <label>Tanggal Pemeriksaan</label>
                    <input type="date" class="p-2 mt-2 w-full border rounded" value="<?=isset($old) && $old["TGL_PEMERIKSAAN"] ? $old["TGL_PEMERIKSAAN"] : $datOP['TGL_PEMERIKSAAN_OP']->format('Y-m-d')?>" name="TGL_PEMERIKSAAN">
                </div>

                <div class="form-group mb-2">
                    <label>NIP Pemeriksa</label>
                    <input type="text" class="p-2 mt-2 w-full border rounded" value="<?=isset($old) && $old["NIP_PEMERIKSA"] ? $old["NIP_PEMERIKSA"] : $datOP["NIP_PEMERIKSA_OP"]?>" name="NIP_PEMERIKSA">
                </div>

                <div class="form-group mb-2">
                    <label>Tanggal Perekam</label>
                    <input type="date" class="p-2 mt-2 w-full border rounded" value="<?=isset($old) && $old["TGL_PEREKAMAN"] ? $old["TGL_PEREKAMAN"] : $datOP['TGL_PEREKAMAN_OP']->format('Y-m-d')?>" name="TGL_PEREKAMAN">
                </div>

                <div class="form-group mb-2">
                    <label>NIP Perekam</label>
                    <input type="text" class="p-2 mt-2 w-full border rounded" value="<?=isset($old) && $old["NIP_PEREKAM"] ? $old["NIP_PEREKAM"] : $datOP["NIP_PEREKAM_OP"]?>" name="NIP_PEREKAM">
                </div>
