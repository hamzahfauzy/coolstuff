...
<div class="content lg:max-w-screen-lg lg:mx-auto py-8">
    ...
    <div class="my-6">
        ...
        <?php endif ?>
        <a href="index.php?page=builder/subjek-pajak/create" class="p-2 bg-green-500 text-white rounded">+ Add New</a>
        <div class="mt-5">
            <form action="" method="get">
                ...
                <div class="form-group inline-block" id="kelurahan">
                    <select name="kelurahan" class="p-2 w-full border rounded" onchange="kelurahanChange(this)">
                        ...
                        <?php foreach($kelurahans as $kelurahan):?>
                            <option <?= isset($_GET['kelurahan']) && $_GET['kelurahan'] == $kelurahan['KD_KELURAHAN']." - ".$kelurahan['NM_KELURAHAN']  ? 'selected' : ''?> value="<?=$kelurahan['KD_KELURAHAN']." - ".$kelurahan['NM_KELURAHAN']?>"><?=$kelurahan['KD_KELURAHAN']." - ".$kelurahan['NM_KELURAHAN']?></option>
                        <?php endforeach ?>
                    </select>
                </div>
                <div class="form-group inline-block <?=(isset($_GET['blok']) && $_GET['blok']) || (isset($_GET['kelurahan']) && $_GET['kelurahan']) ? '' : 'hidden' ?>" id="blok">
                    <select name="blok" class="p-2 w-full border rounded">
                        ...
                        <?php foreach($bloks as $blok):?>
                            <option <?= isset($_GET['blok']) && $_GET['blok'] == $blok['KD_BLOK']  ? 'selected' : ''?> value="<?=$blok['KD_BLOK']?>"><?=$blok['KD_BLOK']?></option>
                        <?php endforeach ?>
                    </select>
                </div>
            ...
            <table class="min-w-max w-full table-auto">
                <tbody class="text-gray-600 text-sm font-light">
                    ...
                    <tr class="border-b border-gray-200 hover:bg-gray-100">
                        ...
                        <td class="py-3 px-6 text-left whitespace-nowrap">
                            <div class="flex items-center">
                                <span class="font-medium"><?=$pekerjaans[$data['STATUS_PEKERJAAN_WP']]?></span>
                            </div>
                        </td>
                        <td class="py-3 px-6 text-center">
                            <div class="flex item-center justify-center">
                                <a href="index.php?page=builder/subjek-pajak/view&id=<?=$data['SUBJEK_PAJAK_ID']?>" class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                    </svg>
                                </a>
                                <a href="index.php?page=builder/subjek-pajak/edit&id=<?=$data['SUBJEK_PAJAK_ID']?>" class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                                    </svg>
                                </a>
                                <a href="index.php?action=builder/subjek-pajak/delete&id=<?=$data['SUBJEK_PAJAK_ID']?>" class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110" onclick="if(confirm('Apakah anda yakin menghapus data ini')){return true}else{return false}">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
