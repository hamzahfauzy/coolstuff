...
$esppts = $mysql->select('esppt')->where('ID_WAJIB_PAJAK',$data['SUBJEK_PAJAK_ID'])->where('STATUS','DITERIMA')->get();
if($esppts)
{
    foreach($esppts as $esppt)
    {
        if (filter_var($esppt['EMAIL'], FILTER_VALIDATE_EMAIL)) {
            ob_start();
            require_once('mail-template/sppt.php');
            $message = ob_get_contents();
            ob_end_clean();
            // send notif accept
            $mail = new Mailer();
            $mail->send($esppt['EMAIL'],'PEMBERITAHUAN e-SPPT',$message);
    
            set_flash_msg(['success'=>'e-SPPT telah dikirim!']);
        }
        else
        {
            set_flash_msg(['error'=>'Email tidak valid!']);
        }
    }
}
else
{
    set_flash_msg(['error'=>'e-SPPT tidak terdaftar!']);
}

$_GET['page'] = 'builder/laporan/sppt/results';
$nop  = $_GET['NOP'];
unset($_GET['NOP']);
$_GET['nop'] = $nop;
